...
function InspectionFormTemplatePage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (v: FormValues & { id?: string }) => {
      const matched = parts.find(
        (p) => p.part_name.toLowerCase() === v.part_name_input.toLowerCase(),
      );
      const partId = matched?.id ?? null;
      const partName =
        v.part_name_input || (matched ? `${matched.part_name} (${matched.part_code})` : v.part_id);
      const fields = {
        field_a: v.field_a || null,
        field_b: v.field_b || null,
        field_c: v.field_c || null,
      };
      if (v.id) {
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await (supabase.from("form_submissions") as any).insert({
        template_id: viewRecord.id,
        form_title: viewRecord.part_name,
        form_number: viewRecord.record_id,
    ...
    mutationFn: async () => {
      ...
      if (aiEditPrefix) {
        const { data } = await (supabase.rpc("next_record_number") as any)({
          p_prefix: aiEditPrefix,
        });
        recordId = `${aiEditPrefix}${String((data ?? 0) as number).padStart(3, "0")}`;
      }
      const { error } = await (supabase.from("inspection_form_templates") as any)
        .update({
          part_name: aiEditName,
          record_id: recordId,
          record_prefix: aiEditPrefix || null,
        })
        .eq("id", aiEditId);
      if (error) throw error;
    },
  ...
  const canSave =
    form.part_name_input && form.record_id && form.tolerance !== "" && Number(form.tolerance) > 0;
                  ...
                  <div className="flex gap-1">
                    ...
                    {t.is_ai_generated ? (
                      <Button
                        ...
                        onClick={() => {
                          setAiEditId(t.id);
                          setAiEditName(t.part_name);
                          setAiEditPrefix(t.record_prefix ?? "");
                          setAiEditOpen(true);
                        }}
                      >
  ...
  return (
    <div>
      ...
      <Card className="overflow-hidden p-0">
        <CardHeader className="py-2.5 px-4 bg-gradient-to-r from-slate-50/80 to-gray-50/80 border-b">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="text-[10px] ml-auto">
              {manualTemplates.length} templates
            </Badge>
          </div>
        </CardHeader>
        <CardContent className="p-0">
      ...
      <Dialog
        open={aiEditOpen}
        onOpenChange={(v) => {
          if (!v) setAiEditOpen(false);
        }}
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Edit AI-Generated Template</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                value={aiEditName}
                onChange={(e) => setAiEditName(e.target.value)}
                className="mt-1"
              />
            </div>
            <div>
              ...
              <Input
                value={aiEditPrefix}
                onChange={(e) => setAiEditPrefix(e.target.value)}
                placeholder="e.g. FORM_PSP_QI_"
                className="mt-1 font-mono"
              />
              <p className="text-[10px] text-muted-foreground mt-1">
                Number auto-generates on save (001, 002…)
              </p>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAiEditOpen(false)}>
              Cancel
            </Button>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => editRecord && saveMutation.mutate({ ...form, id: editRecord.id })}
              disabled={!canSave || saveMutation.isPending}
            >
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-0">
          <DialogHeader className="px-6 pt-4 pb-0">
            <div className="flex items-center justify-between">
              <DialogTitle className="text-sm">
                {fillMode ? "Fill Form" : "Form Preview"}
              </DialogTitle>
              {!fillMode && (
                <Button
                  variant="outline"
                  ...
                  onClick={() => {
                    ...
                    viewRecord?.form_schema?.sections?.forEach((s: any) =>
                      s.fields?.forEach((f: any) => {
                        if (f.prefill_from_template) vals[f.key] = f.prefill_from_template;
                      }),
                    );
                    setFormValues(vals);
                  }}
          ...
          <div className="px-6 pb-4">
            {viewRecord?.form_schema && (
              <FormSchemaRenderer
                schema={viewRecord.form_schema}
                values={formValues}
                onChange={
                  fillMode
                    ? (key, value) => setFormValues((v) => ({ ...v, [key]: value }))
                    : undefined
                }
                readOnly={!fillMode}
              />
